...
const Hero: React.FC = () => {
  ...
  return (
    <section id="home" className="relative min-h-screen flex items-center justify-center px-6 md:px-12 lg:px-24 pt-20 overflow-hidden">
      ...
      <div className="max-w-7xl w-full grid md:grid-cols-2 gap-12 items-center z-10">
        ...
        <div className="order-2 md:order-1">
          ...
          <h1 className="text-5xl md:text-7xl font-bold text-white mb-6 leading-tight tracking-tight animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
            {name}
            <span className="block text-slate-400 mt-2 text-2xl md:text-4xl lg:text-5xl bg-clip-text text-transparent bg-gradient-to-r from-slate-400 to-slate-600">
              {hero.headline}
            </span>
          </h1>
          ...
          <div className="flex flex-wrap gap-4 animate-fade-in-up" style={{ animationDelay: '0.5s' }}>
            ...
            {hero.ctaSecondary && (
              <a
                href="#services"
                className="inline-flex items-center gap-2 px-8 py-4 bg-transparent border border-slate-600 text-white font-bold rounded hover:border-white hover:bg-white/5 transition-all transform hover:scale-105"
              >
                {hero.ctaSecondary}
              </a>
            )}
