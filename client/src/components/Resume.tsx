            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div ref={ref} className="mb-24 px-4 md:px-0">
                    ...
                    <div className="relative ml-4 md:ml-8">
                        {/* Animated line */}
                        <div
                            className={`absolute left-0 top-0 bottom-0 w-px bg-slate-800 ${isVisible ? 'timeline-line-animated' : ''}`}
                        ...
                        <div className="space-y-12">
                            {content.timeline.items.map((item, index) => (
                                <div
                                    key={item.id}
                                    className={`relative pl-8 md:pl-12 group ${isVisible ? 'reveal-visible' : 'reveal-hidden'}`}
                                    style={{ animationDelay: `${300 + index * 200}ms` }}
                                >
                                    {/* Dot */}
                                    <div
                                        className={`absolute -left-[5px] top-1.5 w-2.5 h-2.5 rounded-sm transition-colors ${isVisible ? 'timeline-node' : ''} ${item.isCurrent
